...
> (a spread parameter occupies several argument registers, spilling to the stack
> past them) and **returned via a multi-result signature when the result fits the
> target's return-register budget** (no heap cell), reassembled into the in-cell
> `f64`-slot layout only where it crosses a uniform/generic/first-class boundary —
> so vector/matrix algebra and `(Float, Float)`-returning helpers compute
...
> now). A result *wider* than the return-register budget is likewise returned
> boxed: arguments spill to the stack, but a multi-value return must fit entirely
> in registers, so the cap is the host target's float **return**-register count
> (AArch64 eight, x86-64 System V two, Windows x64 one). Reuse & in-place update (M6) are built:
...
> **dogfooded in the standard library** (`std/testing/Test.fai`: a pure splitmix64 `Gen`,
...
> `game_of_life`) run on them. **Effect-carrying data types and a lazy `Stream`**
> are built: an effect parameter may now ride a user `type`/alias parameter (not
> just an interface's), so a plain data type can store an effectful suspension in
> its type — kind inference is a per-file monotone fixpoint that classifies even
> mutually-recursive types that thread an effect. On that, **`std/io/Stream.fai`** is a
> lazy, pull-based, effect-carrying sequence `Stream 'a 'e` (the strict-ML `Seq`
> shape with an effect row): an opaque `MkStream (Unit -> Step 'a 'e / 'e)` whose
> transformers (`map`/`filter`/`take`/`flatMap`/`zip`/`scan`/…) are pure and
> compose, while terminal consumers (`fold`/`toList`/`forEach`/…) drive the effects
> and return `Result _ String` (an `Err` on a mid-stream `Fail`). It powers
> **progressive file & stdio reading**: `FileSystem` gains chunked handle ops
> (`openRead`/`openWrite`/`openAppend` → opaque `Reader`/`Writer`, `readChunk`/
> `writeChunk`, `closeReader`/`closeWriter`) and `Console` gains `write`/
> `writeError`/`readLine` (stdin), each blocking call parking on the blocking pool
> under the scheduler (direct style, no awaitable type); lazy sources (`fileLines`/
> `ofReader`/`stdinLines`, with `decodeUtf8Lines` buffering UTF-8 across chunks) and
> sinks (`toFile`/`toStdout`/…) bridge the two, and a handle closes when its stream
> is dropped (reference counting). A **NodaTime-style date & time library** is built
> under **`std/datetime/`**: the opaque, validated, nanosecond-precision value types
> `Instant`/`Duration`/`LocalDate`/`LocalTime`/`LocalDateTime`/`Offset`/
> `OffsetDateTime`/`Period` (each stored as integer day/nanosecond counts, the
> calendar via the standard civil↔days conversion) plus the `DayOfWeek`/`Month`
> enums, with ISO-8601 `toString`/`parse` everywhere and a `DateTimeFormat`
> custom-pattern engine. It is pure but for `Instant.now`/`OffsetDateTime.now`,
> which take the `Clock` capability — now also offering `localOffset` (the system
> UTC offset in seconds, a new `fai_clock_local_offset` runtime primitive) so local
> wall-clock time is available; full IANA time zones / `ZonedDateTime` are deferred
> (the library is offset-based). Later
...
| Inference | Hindley–Milner + let-generalization + **rows / row unification / lacks constraints** + **effect rows** (a parallel effect-row union-find over arrows, strict by default with the signature-vs-body check lenient); exhaustiveness checking for `match`. A parameter's **kind** (type vs effect row) is inferred from use — for interfaces *and* user `type`/alias declarations (a per-file monotone fixpoint, so mutually-recursive effect-threading types like `Stream`/`Step` classify correctly); a parameter used as both is **`FAI3019`** |
...
| Standard library | Real compiled `.fai` modules under **`std/`**, embedded at build time. One **auto-imported** module, `Prelude`, owns the core types `Option`/`Result` (with their constructors), re-exports the opaque `Dict`/`Set`/`HashDict`/`HashSet` type names, and provides the free functions `identity`/`const`/`not`/`compare`; all other operations are **qualified** under per-type modules (`List.map`, `Array.map`, `Option.withDefault`, `Int.toString`, …). `Prelude`/`List`/`Array`/`Option`/`Result`/`Dict`/`Set`/`HashDict`/`HashSet`/`String`/`Bytes`/`Int`/`Float`/`Char`/`Concurrency`/`Net`/`Async`/`Stream`/`Io`/`Headers`/`Url`/`Http`/`Tls` — plus the date & time modules `Instant`/`Duration`/`LocalDate`/`LocalTime`/`LocalDateTime`/`Offset`/`OffsetDateTime`/`Period`/`DayOfWeek`/`Month`/`DateTimeFormat` — are reserved module names. **`Bytes`** is a built-in `Con` (global like `String`/`Array`, no import): an immutable contiguous **binary byte buffer**, distinct from the UTF-8 `String`, whose elements are read/written as `Int`s (0–255). It shares the inline `String` buffer representation at runtime but carries a distinct kind (so a binary buffer never compares equal to a `String`); its `Bytes` module offers `fromList`/`toList`, `fromString`/`toString` (the latter `Option String`, guarded by a UTF-8 check), `length`/`get`/`unsafeGet`, `concat`, and `slice`, built on the `Prim.bytes*` intrinsics. It is the payload type for the network capability. **Concurrency** is a capability (in the default `Runtime`): `scope`/`spawn`/`await` for structured tasks (a `scope` opens a `Nursery` and joins every task spawned into it) and bounded `Channel`s (`channel`/`send`/`recv`/`close`); `scope`/`spawn` are effect-polymorphic, forwarding the spawned body's own effect. The opaque `Task`/`Channel`/`Nursery` handle types live in the `Concurrency` module and are re-exported by `Prelude`. **`Async`** is an ordinary library module (not a capability) of high-level combinators over those primitives — `parallel`/`parallel2`/`parallel3` (fan a set of thunks out and join their results), `mapConcurrent`/`iterConcurrent` (a concurrent `List.map`/iteration), and the channel helpers `collect`/`sendAll`/`produceList`/`pipe` — each taking the `Concurrency` capability and opening its own structured scope, so the common patterns are one call rather than a hand-written nursery with manual `spawn`/`await`. It is direct-style (a unit of deferred work is an ordinary thunk `Unit -> 'a`, and a blocking step parks transparently), so it needs no monadic `Async` value or block syntax. **`Net`** is a capability (also in the default `Runtime`): TCP `listen`/`localPort`/`accept`/`connect`/`send`/`recv`/`close` and connectionless UDP `udpBind`/`udpLocalPort`/`udpSend`/`udpRecv`/`udpClose` (each datagram addressed by host/port; `udpRecv` reports the sender) over an async I/O reactor (`mio`, readiness-driven epoll/kqueue/IOCP), with `Bytes` payloads and `Result _ String` results; the opaque `Listener`/`Connection`/`UdpSocket` handle types live in the `Net` module and are re-exported by `Prelude`. Each operation runs on its task and parks on the reactor while it would block (hostname resolution for `connect` runs on the blocking pool), so a server multiplexes many connections onto the worker pool. A program that uses `Net` runs on the scheduler exactly as a concurrent one does (the execution gate triggers on `Concurrency` **or** `Net`). The few Rust **intrinsics** are prelude-private, reached only as `Prim.*` from inside `std/` (`FAI2014` elsewhere) and re-exported under clean names; a **saturated call to such a re-export is inlined to its primitive** (the intrinsic inliner), so the wrapper adds no indirection at a use site. **Two associative-container families:** the ordered `Dict`/`Set` (weight-balanced BSTs, O(log n), sorted iteration/range use) and the unordered `HashDict`/`HashSet` (flat open-addressing hash tables over `Array`, O(1) average, unspecified-but-deterministic iteration), all **`opaque`** types declared in their own modules with their node constructors hidden and re-exported by `Prelude` via transparent aliases. The hash containers build on a structural-hash primitive (`Prim.hash`, the peer of `Prim.compare`) that agrees with structural equality; they keep value semantics like `Array` (copy-on-share, in-place update when uniquely owned). The collection and `Option`/`Result` lookups offer **fused get-or-default accessors** (`Dict.getOr`/`getOrElse`, `Dict.member`, `HashDict.getOr`/`getOrElse`/`member`, `Option.getOrElse`, `Result.withDefault`/`getOrElse`/`toOption`) that return the contained value (or a default) directly, never materializing the intermediate `Option`. **`std/io/Stream.fai`** is a lazy, pull-based, effect-carrying sequence `Stream 'a 'e` (an opaque effect-carrying ADT — see Inference): pure transformers (`map`/`filter`/`take`/`drop`/`flatMap`/`zip`/`scan`/`enumerate`/…) compose, a construction kit (`empty`/`cons`/`defer`/`failed`/`unfold`) and producers (`fromList`/`range`/`iterate`/…) build streams, and terminal consumers (`fold`/`toList`/`forEach`/`find`/…) return `Result _ String` (`Err` on a mid-stream `Fail`). Progressive **file/stdio I/O** layers on it: `FileSystem` grows chunked handle ops (`openRead`/`openWrite`/`openAppend` → the opaque `Reader`/`Writer` handle types in the dependency-free **`Io`** module, re-exported by `Prelude`; `readChunk`/`writeChunk`, an empty `Bytes` is EOF; `closeReader`/`closeWriter`), and `Console` grows `write`/`writeError`/`readLine` (stdin) — each blocking call parks on the blocking pool inside a task and runs inline otherwise (the `readFile` dispatch), so streaming I/O is transparently async with no awaitable type. Lazy sources (`fileLines`/`ofReader`/`fileBytes`/`stdinLines`, `decodeUtf8Lines` buffering UTF-8 across chunks) and sinks (`toWriter`/`toFile`/`appendToFile`/`toFileText`/`writeLinesToFile`/`toStdout`/`toStdoutLines`/`toStderr`) bridge streams and I/O; a handle closes when its stream is dropped (reference counting). A `KIND_FILE` runtime cell owns the `BufReader`/`BufWriter` like the `Net` handles. A **NodaTime-style date & time library** lives under **`std/datetime/`** (each type its own module, the names re-exported by `Prelude`): the **opaque, validated value types** `Instant` and `Duration` (a UTC timeline point and a fixed elapsed length, each nanosecond-precision and stored as a whole-day count plus a nanosecond-of-day, so the range is vast), `LocalDate` (an epoch-day count, with the civil↔days calendar conversion), `LocalTime` (a nanosecond-of-day), `LocalDateTime`, `Offset` (whole seconds from UTC, ±18h), `OffsetDateTime` (a local date-time plus an offset, compared by instant), and the calendar-aware `Period` (independent year…nanosecond components, applied with `addToDate`/`addToDateTime`), plus the `DayOfWeek`/`Month` enums (constructors reached qualified). Each type renders and reads ISO-8601 (`toString`/`parse`), and **`DateTimeFormat`** adds a custom pattern mini-language (`yyyy`/`MM`/`MMMM`/`dd`/`HH`/`hh`/`mm`/`ss`/`fff`/`tt`/`EEEE`, quoted literals). It is pure but for `Instant.now`/`OffsetDateTime.now`, which take the `Clock` capability — extended with `localOffset : Unit -> Int / { Clock }` (the system UTC offset in seconds, backed by the `fai_clock_local_offset` runtime primitive) so local wall-clock time is available. Full IANA time zones / `ZonedDateTime` (with daylight-saving rules) are deferred; the library is offset-based. The concurrency surface is rounded out with a **scheduler timer** and **task cancellation**: `Clock` gains `sleep` (parks a task on the reactor timer, or sleeps the thread when run inline), `Concurrency` gains `cancel` (cooperative, sticky cancellation that propagates down the task tree — a cancelled task's next blocking call returns a cancellation `Err` so it unwinds), and `Async` gains `timeout`/`race` built on them. A **`Tls`** capability (in the default `Runtime`) wraps a sans-I/O **rustls** engine (the `ring` provider): `client`/`clientWithRoots`/`server` start a session and `feedIncoming`/`takeOutgoing`/`readPlaintext`/`writePlaintext`/`state` step it over in-memory buffers, so Fai drives the handshake and shuttles ciphertext over the existing `Net` transport (the opaque `TlsSession` handle lives in the `Tls` module, re-exported by `Prelude`). On these, **`std/networking/Http.fai`** is a pure-Fai HTTP/1.1 client and server (over `Net`, with HTTPS over `Tls`): a `Request`/`Response` carry a parsed **`Url`** (opaque, `std/networking/Url.fai`: scheme/userinfo/host/port/path/query/fragment, parse/resolve/`toString`, percent-encoding), case-insensitive **`Headers`** (opaque, `std/networking/Headers.fai`, order- and duplicate-preserving), and a streaming `Stream Bytes` body; framing is written against an abstract `Transport` (recv/send/close), with a **`plainTransport`** over a TCP `Connection` and a **`tlsTransport`** that drives the TLS handshake and shuttles ciphertext over the same `Connection`. The client (`get`/`getWith`/`post`/`postForm`/`request`/`requestWith`/`requestOnce`, connection-per-request) picks plain-vs-TLS by the URL scheme at runtime, so its surface is uniformly `/ { Net, Tls }` (`plainTransport` over-declares the `Tls` effect it never performs so the two transports unify in the scheme branch); `getWith`/`requestWith` take extra trusted roots (a private CA or a self-signed server). It **follows redirects** automatically (up to a hop limit): a 301/302/303 becomes a `GET`, a 307/308 is followed only for a bodyless method (the consumed body cannot be replayed), a relative `Location` is resolved against the request URL, and `Authorization` is dropped on a cross-origin hop (along with the hop-specific `Host`/`Content-Length`/`Transfer-Encoding`); each redirect response is dropped (closing its connection) before re-requesting, and `requestOnce` is the single-shot, no-follow escape hatch. **Authorization and form helpers** round out the request side — `basicAuth`/`bearer` build an `Authorization` header value (over a `base64Encode`), `formBody`/`postForm` build an `application/x-www-form-urlencoded` body, and `field`/`filePart`/`multipartBody`/`postMultipart` build a `multipart/form-data` body (each `Part` carrying an optional filename and content type, framed with a `randomBoundary`). The server (`serve`/`serveListener` for HTTP, `serveTls`/`serveListenerTls` presenting a PEM cert+key for HTTPS) runs a task per connection with graceful shutdown via cancellation. A **received** body is a **lazy `Stream Bytes`** decoded on demand per its framing — **chunked** transfer-encoding, `Content-Length`, or read-to-EOF — so a client streams a large response without buffering it whole (the body owns the connection, which closes when the body is dropped); a server request body is read in full so the transport is free for the response. A **sent** body is framed with `Content-Length` (the stream is drained first) by default, or streamed **chunked** without buffering when its headers select `Transfer-Encoding: chunked` (`chunkedResponse`, or the header on a request) — driven a piece at a time by `Stream.uncons`, which unions the body's own effect with the transport's `{ Net, Tls }` (a `fold`/`forEach` consumer cannot, as it ties the element action and the stream to one effect). A **connection-pooling client** layers on the same machinery: `withClient` opens a structured scope holding a pool actor (a task that owns the idle keep-alive connections, reached over one command channel) and hands the callback an opaque `Client`; `getOn`/`requestOn`/`postOn` reuse an idle connection to the request's origin (scheme/host/port) when one is available and open a fresh one otherwise (a pooled request is sent keep-alive rather than `Connection: close`), and a pooled response body returns its connection to the pool when fully read — closing it instead on an early drop or a read error. A reused connection that fails before any response byte is retried once on a fresh connection, and every connection is closed when the scope exits. HTTP/2 and a cross-`withClient` shared pool are noted as follow-up work. |
...
├── std/                 # standard library: real .fai modules (grouped into subfolders), embedded at build time
│   ├── core/            # auto-imported core: Prelude, Option, Result
│   ├── collections/     # List, Array, Dict, Set, HashDict, HashSet
│   ├── primitives/      # Int, Float, Char, String, Bytes
│   ├── networking/      # Net, Http, Tls, Url, Headers
│   ├── concurrency/     # Concurrency, Async
│   ├── io/              # Io, Stream
│   ├── testing/         # Test (the property-testing framework)
│   └── datetime/        # NodaTime-style date & time modules (Instant, LocalDate, …)
├── editors/
...
│   ├── fai-runtime/     # Rust lib: RC (biased), allocator, builtins, capability hosts, M:N scheduler (M3). Cargo-built archive (has native deps)
...
declarations in `std/core/Prelude.fai`, no longer a closed compiler enum.
