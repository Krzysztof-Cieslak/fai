...
/// A type or interface parameter is used as both a type and an effect row (an
/// ill-kinded parameter).
pub const INTERFACE_PARAM_KIND: DiagnosticCode = DiagnosticCode::new("FAI3019");
/// A type/interface argument has the wrong kind — an effect row where a type is
/// expected (or vice versa), or an effect row written outside an argument
/// position.
...
pub const CODES: &[CodeInfo] = &[
    ...
    CodeInfo {
        ...
        title: "parameter used as both a type and an effect",
        default_severity: Severity::Error,
        explanation: "A type or interface parameter (`'a`) is used in type position and also as an \
                      effect row (after `/`). A parameter is one kind or the other — give the type \
                      use and the effect use separate parameters.",
    },
    CodeInfo {
        code: EFFECT_ARG_KIND,
        title: "wrong kind of type argument",
        default_severity: Severity::Error,
        explanation: "A type or interface argument has the wrong kind: an effect row (`{ Console }`) \
                      was supplied for a type parameter, a type for an effect parameter, or an \
                      effect row was written somewhere other than an argument position. Supply the \
                      kind the parameter expects.",
    },
    CodeInfo {
