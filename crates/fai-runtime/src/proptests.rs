...
#[test]
fn prop_allocator_invariants_hold_over_random_op_sequences() {
    // Each generated byte string is a sequence of alloc/free ops over sizes that
    // span the pooled classes and the large fallback. `run_ops` checks no-alias,
    // alignment, and payload integrity after every step (panicking, which proptest
    // shrinks); the balance assertion confirms nothing is left live.
    check(proptest::collection::vec(any::<u8>(), 0..512), |data| {
        let base = live_count();
        run_ops(&data);
        prop_assert_eq!(live_count(), base, "the op sequence freed everything");
        Ok(())
    });
}
